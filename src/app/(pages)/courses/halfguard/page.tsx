...
export default function HalfGuardPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      <section
        id="hero"
        className="bg-linear-to-b from-half-guard-theme-light via-half-guard-theme to-background dark:bg-background pt-40 pb-12 lg:pb-20 text-foreground"
      >
        <div className="w-full flex justify-center items-center">
          <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 items-center gap-4 px-4 lg:px-48">
            <div className="text-center lg:text-left">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4">
                {t.halfguard.hero.title}
              </h1>
              <p className="text-xl text-foreground mb-8">
                {t.halfguard.hero.subtitle}
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start mx-2 lg:mx-auto">
                ...
                >
                  ...
                  <p className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-half-guard-theme px-4 text-md font-medium text-foreground backdrop-blur-3xl">
                    {t.halfguard.hero.cta}
                  </p>
                </motion.a>
              </div>
            ...
            <VideoPlayer
              ...
              videoTitle={t.halfguard.hero.videoTitle}
              videoDescription={t.halfguard.hero.videoDescription}
              autoPlay
            />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-4 lg:mx-auto">
            {t.halfguard.benefits.items.map(
              (
                ...
                <motion.div
                  key={index}
                  className="space-y-1.5 p-6 rounded-lg border border-half-guard-theme/50 shadow-sm "
                  whileHover={{
                    scale: 1.05,
                    transition: { duration: 0.3 },
                  }}
      ...
      <section
        id="callAction"
        className="py-8 lg:py-16 bg-half-guard-theme-dark text-foreground"
      >
        <div className=" text-center">
          <h2 className="text-3xl md:text-4xl font-semibold mb-6">
            {t.halfguard.cta.title}
          </h2>
          <p className="text-xl lg:text-2xl mb-4 max-w-2xl mx-auto">
            {t.halfguard.cta.from} <s className="text-foreground">R$ 197,00</s>
          </p>

          <div className="mb-8">
            <div className="text-5xl font-bold mb-2 text-foreground">
              {t.halfguard.cta.price}
            </div>
            <div className="text-xl text-foreground">
              {t.halfguard.cta.installments}
            </div>
          </div>
        ...
        <div className=" ">
          <div className="max-w-6xl mx-4 lg:mx-auto text-center">
            <h2 className="text-3xl text-foreground font-semibold mb-2">
              {t.halfguard.guarantee.title}
            </h2>
            <p className="text-lg text-muted-foreground mb-8">
              {t.halfguard.guarantee.subtitle}
            </p>

            <div className="bg-background/50 rounded-xl shadow-xl p-8 mb-8 border-t-4 border-half-guard-theme-dark">
              <div className="flex flex-col items-center mb-6">
                ...
                >
                  <RiShieldCheckLine size={68} fill="#916600" />
                </motion.div>
                <h3 className="text-2xl font-bold text-foreground">
                  {t.halfguard.guarantee.seal}
                </h3>
              </div>

              <p className="text-foreground leading-relaxed mb-6">
                {t.halfguard.guarantee.description}
              </p>

              <div className="p-4 bg-foreground/5 rounded-lg border border-foreground/20">
                <p className="font-medium text-foreground">
                  {t.halfguard.guarantee.noQuestions}
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="faq" className="py-8 lg:py-16 bg-background">
        <div className="">
          <div className="max-w-3xl mx-4 lg:mx-auto">
            <h2 className="text-3xl text-half-guard-theme font-semibold text-center mb-8">
              {t.halfguard.faq.title}
            </h2>

            <FaqSection items={t.halfguard.faq.items} />

            <div className="text-center mt-8">
              <h3 className="text-xl text-half-guard-theme font-bold mb-2">
                {t.halfguard.faq.stillDoubts}
              </h3>
              <p className="text-muted-foreground mb-6">
                {t.halfguard.faq.description}
              </p>
              <button
      ...
      <OtherCoursesSection currentCourse="halfguard" />
      <ScrollToTopButton />
    </main>
  );
}
